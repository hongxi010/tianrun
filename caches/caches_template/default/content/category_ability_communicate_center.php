...
<div class="solution ">
    ...
    <div class="section_d">
        <div class="autowidth animationDH animateBt current">
            ...
            <div class="font14 color666 centers mb6">
                <?php echo $li['content'];?>
            </div>
            ...
            <?php $n++;}unset($n); ?>
            <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
            <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=5c7c8853ce55e63dd054195939d510f9&action=lists&catid=%24s%5Bcatid%5D&where=listorder%3E1&num=10&order=listorder+ASC&moreinfo=1&return=list\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$content_tag = bc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {global $list; $list = $content_tag->lists(array('catid'=>$s[catid],'where'=>'listorder>1','order'=>'listorder ASC','moreinfo'=>'1','limit'=>'10',));}?>
    ...
    <div class="section_d  bgc">
        <div class="autowidth animationDH animateBt">
            ...
            <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=3060001211dcd7bb6dc8058891d17962&action=lists&catid=%24s%5Bcatid%5D&num=1&order=listorder+ASC&moreinfo=1&return=list\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$content_tag = bc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {global $list; $list = $content_tag->lists(array('catid'=>$s[catid],'order'=>'listorder ASC','moreinfo'=>'1','limit'=>'1',));}?>
            <?php $n=1;if(is_array($list)) foreach($list AS $li) { ?>
            <div class="font16 font400 mb6 centers color666">
                <?php echo $li['description'];?></div>
            <?php $n++;}unset($n); ?>
            <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
            <ul class="flex flexwp flexbt font18 font400 centers ullimb3 ultype">
                <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=720fdbe0741c9738755da826f6621cc9&action=lists&catid=%24s%5Bcatid%5D&where=listorder%3E1&num=30&order=listorder+ASC&moreinfo=1&return=list\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$content_tag = bc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {global $list; $list = $content_tag->lists(array('catid'=>$s[catid],'where'=>'listorder>1','order'=>'listorder ASC','moreinfo'=>'1','limit'=>'30',));}?>
                <?php $n=1;if(is_array($list)) foreach($list AS $li) { ?>
                <li class="w23 flex flexat flexjt  height64 bgfff mb3"><?php echo $li['title'];?></li>
                <?php $n++;}unset($n); ?>
                <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
                <li class="w23 flex flexat flexjt  height64 bgfff mb3">
                    <div class="publicBtn">
