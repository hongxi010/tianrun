	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE'), $time, bc_base::load_config('system','cookie_path'), bc_base::load_config('system','cookie_domain'), $s,true);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE'), $time, bc_base::load_config('system','cookie_path'), bc_base::load_config('system','cookie_domain'), $s,true);
		}
	}
