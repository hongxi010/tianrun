...
class index {
	...
	public function init(){
		...
		if($id){
			...
			if($data){
				...
				$tagdata = $this->db_content->listinfo($where,$sql_ord, $page, 12);
	...
	public function ajaxlists(){
		$id = intval($_GET['id']);
		$tagdir = remove_xss($_GET['tag']);
		$tagdir = safe_replace($tagdir);
		$models = getcache('model', 'commons');
		$sitelist = getcache('sitelist', 'commons');
		$i=0;
		$siteid = intval($_GET['siteid']);
		...
		if($id || $tagdir){
			if($id){
				...
				$data = $this->db->get_one(array('tagdir'=>$tagdir));
			}
			if($data){
				$tag = safe_replace($data['tag']);
				$sql_arr = array('tag'=>$tag);
				...
				$tagrootdir = $html_root.$tag_root;
				$urlrule = $tagrootdir.'/'.$tagdir.'/~'.$tagrootdir.'/'.$tagdir.'/index_{$page}.html';
				$tagdata = $this->db_content->listinfo($sql_arr,$sql_ord, $page, 12, '', 12, $urlrule);
				$pages = $this->db_content->pages;
				$total = $this->db_content->number;
			}else{
				showmessage('标签不存在！');
