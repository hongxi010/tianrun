...
class formguide_info extends admin {
	...
	public function init() {
		...
		$right_menu = array('?m=formguide&c=formguide_info&a=export_excel&formid='.$formid.'&pc_hash='.$_GET['pc_hash'],'导出Excel(全部)');
        $right_menu2 = array('?m=formguide&c=formguide_info&a=export_excel&formid='.$formid.'&page='.$page.'&bypage=1','导出Excel(分页)');
		if($formid==38){
			include $this->admin_tpl('formguide_info_list_38');
		}else{
	...
	public function  export_excel(){
		...
		$this->db->change_table($this->tablename);
// 		$info = $this->db->listinfo();
        $info = $this->db->listinfo('','userid DESC',0,100000);
        $bypage = isset($_GET['bypage']) && $_GET['bypage'] ? $_GET['bypage'] : 0;
        if ($bypage){
            $page = max(intval($_GET['page']), 1);
            $r = $this->db->get_one(array(), "COUNT(dataid) sum");
            $total = $r['sum'];
            $pages = pages($total, $page, 20);
            $offset = ($page-1)*20;
            $info = $this->db->select(array(), '*', $offset.', 20', '`dataid` DESC');
        }
		$j=2;
		foreach($info as $data){
			...
			foreach($data as $f =>$v){
				...
				$resultPHPExcel->getActiveSheet()->setCellValue($arr[$i].$j, $v); 
				$i++;
			}
			$j++;
		}
//		echo json_encode($info);exit;
	...
	public function public_answer() {
        $did = $_POST['did'];
        $answer = $_POST['answer'];
        $answertime = time();  //这里增加了一个时间
        $re = $this->db->query("update bc_form_touzizhe_liuyan set answer='$answer',mark='已回答',answertime=$answertime where dataid=$did");
        echo $re;
    }
}
?>